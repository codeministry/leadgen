...
| `LLM_MODEL_EMBEDDING`       | —       | Read by deduplication's two similarity strategies. **No fallback**: a chat model is not an embedding model, so unset means only `exact_fingerprint` runs. Must return 768-dimensional vectors, which is what the `offer.embedding` column holds.                                                                                                                                                                          |
