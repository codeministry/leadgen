...
- [`CLAUDE.md`](CLAUDE.md) — the working notes: the repo-wide invariants, the file inventory
  and the measured baseline. Written for an AI agent working in the tree. The conventions and
  the traps sit beside the code they are about, in [`backend/CLAUDE.md`](backend/CLAUDE.md)
  and [`frontend/CLAUDE.md`](frontend/CLAUDE.md).
- [`docs/decisions/`](docs/decisions/) — the reasoning behind each stage: every rule with the
  measurement behind it, and why the obvious alternative was not taken. Together with
  `CLAUDE.md` the most complete description of why this code looks the way it does.
