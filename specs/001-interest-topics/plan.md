...
`score_model` and `ruleset_version` unchanged. It runs at the top of every `SCORE` stage when the profile's snapshot
digest differs from the one recorded at the last run, before `DUE` is evaluated, so a topic edit is reflected on the
...
| `J/score/ScoringService.java`                                                                                                                  | `rescoreRules(snapshot)` loop described above; profile digest recorded per run; invoked from the `SCORE` stage before `DUE`                                                                                                                                                       | ISC-197.2                                  |
...
| `J/retrieval/SemanticFilter.java`, new `J/retrieval/TopicEmbeddings.java`                                                                      | stage 5 only: `OR o.retrieval_embedding <=> :topicVec <= :maxDistance` appended to the topic predicate when a model is configured; cache keyed by `(model, digest(text))` copied from `ProfileEmbeddings`; no model → the alias half alone, never `RetrievalUnavailable`          | ISC-196.1, ISC-196.2, ISC-201.2            |
...
| `docs/samples/measure_embeddings.ts`                                                                                                           | phrase mode: embeds a list of phrases and reports cosine against every advert, with a labelled-sample summary                                                                                                                                                                     | ISC-196.1                                  |
...
| `pipeline_run` (or the existing run record) | `ruleset_version`, `score_model`                                                             | `+ profile_digest text NULL`, the profile snapshot the run scored against; drives the rules-only rescore |
...
2. **Migrate.** Nothing to backfill: topic rows appear as the working set is rescored, which the rules-only rescore
   does on the first run after deploy (the profile digest of the last run is `NULL`, so every offer is re-totalled).
...
- fog (spec `## Not yet specified`): the similarity floor for a topic phrase against an advert. Resolved by the stage 5
  measurement; the plan reserves the `TopicEmbeddings` seam and writes nothing into it until the number exists.
...
| BE-ARCH-01..03 vertical slices | leaves it: `TopicEmbeddings` sits in `retrieval/`, the rescore in `score/`, nothing crosses a package it did not already cross |
